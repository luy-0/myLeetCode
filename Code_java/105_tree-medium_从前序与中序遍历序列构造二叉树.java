...
/*
 * 从中序与前序遍历序列构造二叉树 前序的第一个数字是根节点,然后依次是其左子树和右子树 中序的中间某个数组是根节点,左右依次是其左子树和右子树
 * 
 */

/**
 * 版本1 是递归解法,但是在inorder序列中查找根节点时暴力查找,时间浪费大
 */
// class Solution {
...
/**
 * 版本2 依旧是递归版本,但是在查找中序序列中的根节点时使用了哈希,相对更快.
 */
class Solution {
    ...
    public TreeNode myBuildTree(int[] preorder, int[] inorder, int preorder_left, int preorder_right, int inorder_left,
        ...
        node.left = myBuildTree(preorder, inorder, preorder_left + 1, preorder_left + size_left_tree, inorder_left,
                inRoot - 1);
        node.right = myBuildTree(preorder, inorder, preorder_left + size_left_tree + 1, preorder_right, inRoot + 1,
                inorder_right);
        return node;
    }
